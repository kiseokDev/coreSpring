...
//            boolean contains = arrayList.contains(s[1]);
    ...
    public int[] solution(String[] id_list, String[] reports, int k) {
        System.out.println("reports = " + Arrays.toString(reports));
//        stream(reports).forEach(x->insertReportedPlayerName(x));
        stream(reports).forEach(x->insertReportedCount(x));
        System.out.println("reportedCount = " + reportedCount);
        System.out.println("reportedNames = " + reportedNames);
        Map<String, ArrayList> suspendedId = getSuspendedId(k, id_list, reportedNames);
        int[] result = getResult(id_list, suspendedId);
        System.out.println("suspendedId = " + suspendedId);
...
//        int[] answer = {};
    ...
    private void insertReportedPlayerName(String x) {
        String[] s = x.split(" ");
